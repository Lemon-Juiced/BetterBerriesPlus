...
public class MulberryBushBlock extends AbstractBerryBushBlock {
    public MulberryBushBlock(Properties properties) {
        super(properties);
        this.registerDefaultState(this.stateDefinition.any().setValue(AGE, Integer.valueOf(0)));
    }

    public ItemStack getCloneItemStack(BlockGetter blockGetter, BlockPos blockPos, BlockState blockState) {
        return new ItemStack(BetterBerriesPlusItems.MULBERRIES.get());
    }
    ...
    public InteractionResult use(BlockState blockState, Level level, BlockPos blockPos, Player player, InteractionHand interactionHand, BlockHitResult blockHitResult) {
        ...
        if (!flag && player.getItemInHand(interactionHand).is(Items.BONE_MEAL)) {
            ...
            popResource(level, blockPos, new ItemStack(BetterBerriesPlusItems.MULBERRIES.get(), j + (flag ? 1 : 0)));
