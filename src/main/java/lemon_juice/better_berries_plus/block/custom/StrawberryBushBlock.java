...
public class StrawberryBushBlock extends AbstractBerryBushBlock {
    ...
    public ItemStack getCloneItemStack(BlockGetter blockGetter, BlockPos blockPos, BlockState blockState) {
        return new ItemStack(BetterBerriesPlusItems.STRAWBERRIES.get());
    }
    ...
    public InteractionResult use(BlockState blockState, Level level, BlockPos blockPos, Player player, InteractionHand interactionHand, BlockHitResult blockHitResult) {
        ...
        if (!flag && player.getItemInHand(interactionHand).is(Items.BONE_MEAL)) {
            ...
            popResource(level, blockPos, new ItemStack(BetterBerriesPlusItems.STRAWBERRIES.get(), j + (flag ? 1 : 0)));
